...
class PlayerScore(ComponentBase):
    def init(self, id, rank, real_rank):
        self.player_id = id
        board_top_padding = (ScreenSize[1] - height * len(game.players)) / 2 + rank * height + 20
        text_center_x = ScreenSize[0] / 2
        ...
        self.children.create_component('Rectangle', ScreenSize[0] - 2 * x_margin, height, x_margin, board_top_padding)
