...
class AlterMoneyNearestEvent(Tech):
...
class AlterMoneyIfOwnEvent(Tech):
...
class EveryOneAlterMoneyIfOwnEvent(Tech):
...
class RandomAlterMoneyEvent(Tech):
    """觸發玩家隨機獲得/失去一定數量的錢
...
class EveryoneAlterMoneyEvent(Tech):
    """所有玩家獲得/失去一定數量的錢
...
class TransferMoneyToNextEvent(Tech):
    """觸發玩家給下一位玩家一定量的錢
...
class EveryoneTransferMoneyEvent(Tech):
    """所有玩家給玩家 A 一定量的錢
...
class RaceGainOrRemoveTableEvent(Tech):
...
class RandomBuyStandEvent(Tech):
...
class RandomBuyOthersStandEvent(Tech):
